...
export default function ChatRoom({
  ...
  let userFromSession = session?.data?.user?.user;
      ...
      const payload = {
        ...
        toUserId:
          session?.data?.user?.user?.roleId === USER_ROLE.doctor
            ? selectedInbox?.patientId
            : selectedInbox?.doctorId,
  ...
  const handleChangeUpload = async (e) => {
    ...
    for (const file of files) {
      ...
      if (response?.data?.succeeded) {
        ...
        dispatch(
          onFailure({
            message: `pload failed: The file "${file.name}" exceeds the maximum allowed size of ${MAX_FILE_SIZE_MB} MB.`,
          })
        );
        console.error(
          `pload failed: The file "${file.name}" exceeds the maximum allowed size of ${MAX_FILE_SIZE_MB} MB.`
        );
      }
    }
            ...
            >
              <Box display="flex" alignItems="center">
                <Avatar
                  ...
                  alt="Remy Sharp"
                  src={userFromSession?.profilePicture || "N/A"}
                />
                <MuiTypography variant="body1" component="h6" fontWeight="500">
                  {userFromSession?.name || "N/A"}
                </MuiTypography>
              </Box>
            </Box>
          </Box>
          ...
          {uploadMediaListLoading ? (
            ...
            <Box
              sx={{
                overflowX: "auto",
                whiteSpace: "nowrap",
                width: "20em", // or adjust as needed
                ml: 2,
              }}
            >
