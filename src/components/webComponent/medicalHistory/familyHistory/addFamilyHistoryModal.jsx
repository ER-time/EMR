...
const AddFamilyHistoryModal = ({
  ...
  const getAllLookup = useGetLookupByValueQuery("FamilyRelation");
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={4}>
            <Box width="100%">
              ...
              >
                {getAllLookup?.data?.data?.map((familyHistory, index) => (
                  <MenuItem key={index} value={familyHistory?.lookupId}>
                    {familyHistory?.value}
                  </MenuItem>
                ))}
              </Select>
