...
const columns = [
  ...
  {
    header: "prescriptionStartTime",
    accessorKey: "prescriptionStartTime",
    accessorFn: (row) => {
      const { date } = extractDateTimeComponents(row?.prescriptionStartTime);
      return <div>{moment(date).format("MM/DD/YY")}</div>;
    },
  },
  {
    header: "prescriptionEndTime",
...
export default function AddSoapNotesModal({
  soapNoteId,
  open,
  handleClose,
  title,
  ...
  const [deleteSoapNotes, { data, isloading }] = useDeleteSoapNotesMutation();
  ...
  const onSubmit = async (values, { resetForm }) => {
    try {
      let finalPayload = {
        soapNoteId: soapNoteId ? soapNoteId?.soapNoteId : 0,
  ...
  const validationSchema = Yup.object({
    ...
    assessment: Yup.string().required("Field is Required!"),
  ...
  useEffect(() => {
    if (soapNoteId) {
      formik.setValues({
        subjective: soapNoteId?.subjective,
        objective: soapNoteId?.objective,
        assessment: soapNoteId?.assessment,
        plan: soapNoteId?.plan,
      });
    }
  }, [soapNoteId  ])
