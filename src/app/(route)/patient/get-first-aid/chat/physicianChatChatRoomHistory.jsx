...
import { useGeneratePrescriptionFromChatSessionMutation } from "@/redux/slices/chat";
...
export default function PhysicianChatChatRoomHistory({
  showMessanger,
  setShowMessanger,
  selectedInbox,
  messageArray,
  ...
  const [generatePrescriptionFromChatSession, { data, isLoading }] =
  ...
  const sendMessageHandler = async () => {
    try {
      ...
      setInputStr("");
      await generatePrescriptionFromChatSession({
        user_query: inputStr,
        user_id: userID,
        session_id:
          selectedInbox !== null
  ...
  const handleConfirm = async () => {
    try {
      ...
      console.log("response:::", specificChatHistory);
      ...
      if (response?.status === 200) {
        ...
        const DoctorList = await axios.post(
          `https://medical-aibe.xeventechnologies.com/generate_general_physician_chat_title?user_id=${userID}&session_id=${
            selectedInbox !== null
              ? selectedInbox.session_id
              : randomNo?.data?.[0]?.session_id
          }`
  ...
  return (
    <>
      ...
      >
            ...
            {messageArray.map((item, index) => (
              ...
              >
                ...
                {item.type === "outgoing" && (
                  ...
                  >
                    {session?.data?.user?.user?.name?.substring(0, 1) || "P"}
                  </Avatar>
                )}
                <Box>
                  <Box
        ...
        {/* Display the message input field only if there are history entries and is_chat is false */}
        {isChatEnded === false ? (
