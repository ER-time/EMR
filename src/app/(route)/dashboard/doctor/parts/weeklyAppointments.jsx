...
import { useDoctorTotalEarningDataQuery } from "@/redux/slices/dashboard";
...
const WeeklyAppointments = () => {
  ...
  const getTotalEarningData = useDoctorTotalEarningDataQuery({
  ...
  useEffect(() => {
    if (getTotalEarningData.isSuccess && getTotalEarningData.data) {
      const data = getTotalEarningData.data.data || [];
      const chartData = data.map((item) => ({ x: item.date, y: item.count }));
      setEarningData(chartData);
    }
  }, [
    getTotalEarningData.isSuccess,
    getTotalEarningData.data,
    selectedInterval,
  ]);
  ...
  const formatXAxisLabels = (date, index) => {
    switch (selectedInterval) {
      ...
      case "month":
        return index % 5 === 0 ? moment(date).format("D") : ""; // Display day of the month for every 5th label
      case "year":
  ...
  const options = {
    ...
    xaxis: {
      categories: getTotalEarningData.data
        ? getTotalEarningData?.data?.data?.map((item, index) =>
  ...
  return (
    <>
      ...
      <StyledBox width="100%" display="flex" justifyContent="center" py="20px">
        <ReactApexChart
          options={options}
          series={[{ name: "Earnings", data: earningData }]}
          type="bar"
          width={470}
          height={320}
        />
