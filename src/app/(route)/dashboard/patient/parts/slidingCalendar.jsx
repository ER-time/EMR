...
export default function SlidingCalendar({ activeDate, setActiveDate }) {
  ...
  // console.log("startDate", activeDate);
  ...
  const goToNextMonth = () => {
    ...
    nextMonthStartDate.setMonth(startDate.getMonth() + 1);
  ...
  const [filteredPatientAppointment, setFilteredPatientAppointment] = useState(
    getPatientAppointments
  );

  const handleFilter = (date) => {
    const filteredData = getPatientAppointments?.filter((item) => {
      return moment(item.startDate).format("ll").includes(date);
    });

    setFilteredPatientAppointment(filteredData);
  };
  ...
  const handleChange = (event) => {
    const [monthIndex, year] = event.target.value.split("-");

    // Adjust the month index by subtracting 1 to account for zero-based indexing
    const adjustedMonthIndex = parseInt(monthIndex, 10) - 1;

    const newActiveDate = new Date(year, adjustedMonthIndex, 1);

    setActiveDate(newActiveDate);

    const firstDayOfMonth = new Date(year, adjustedMonthIndex, 1);
      ...
      <div className="week-view-calendar">
        <div style={{ position: "relative" }}>
          ...
          <Box className="week-days-row">
            {Array.from(
              { length: screenWidth < 575 ? 1 : screenWidth < 668 ? 3 : 6 },
              (_, i) => {
